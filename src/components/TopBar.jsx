...
export default function TopBar({ user, logout, mode, setMode, zones, isOffline, backendConnected, trackedPhone, setTrackedPhone }) {
    ...
    >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, color: 'var(--clr-text-primary)', lineHeight: 1 }}>
              {totalPopulation >= 1000 ? `${(totalPopulation / 1000).toFixed(1)}K` : totalPopulation}
            </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, fontWeight: 800, color: backendConnected ? 'var(--clr-safe)' : isOffline ? 'var(--clr-warn)' : '#94a3b8', letterSpacing: '0.06em' }}>
            {backendConnected ? 'SYNCED' : isOffline ? 'OFFLINE' : 'PARTIAL'}
          </div>
          <div style={{ fontSize: 7, color: 'var(--clr-text-muted)', letterSpacing: '0.06em' }}>DATA</div>
        </div>
        ...
        {user && (
          <div style={{
            ...
            <div style={{ fontSize: 8, fontWeight: 700, color: 'var(--clr-text-muted)', letterSpacing: '0.04em' }}>TRACK:</div>
            <input 
              type="text" 
              ...
              style={{
                background: 'rgba(255,255,255,0.5)', border: '1px solid var(--clr-border)', 
                borderRadius: 4, padding: '2px 4px', fontSize: 10, width: 90,
                outline: 'none', color: 'var(--clr-text-primary)'
              }}
            />
          </div>
        )}
        ...
        {user && (
          <div style={{
            ...
            <div style={{ width: 22, height: 22, borderRadius: '50%', background: 'linear-gradient(135deg, #3b6ef8, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <User size={11} color="white" />
            </div>
            <div>
              <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--clr-text-primary)', lineHeight: 1 }}>{user.name}</div>
              <div style={{ fontSize: 7, color: 'var(--clr-text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{user.role}</div>
            </div>
            {logout && (
              <button onClick={logout} style={{
                background: 'none', border: 'none', cursor: 'pointer',
                fontSize: 9, color: '#ef4444', fontWeight: 700, marginLeft: 4,
              }}>✕</button>
            )}
          </div>
        )}
